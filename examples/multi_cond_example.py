...
def main():
    ...
    # These are the test condition that we need to append to rnn output so that we can ultimately append them with target
    test_cond_1 = train_cond_1 = create_conditions(input_dim=COND_1_DIM)
    # We're trying with two condition to make sure that appending with rnn output really makes a difference. 
